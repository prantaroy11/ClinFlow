...
The Notification Service saves the email ticket to a dedicated MongoDB database with a status of PENDING.

A scheduled Cron Job (running every 2 minutes) sweeps the database for PENDING tickets.

The cron job dispatches the emails using the Resend API.

Upon successful delivery, the ticket status is updated to SUCCESS.
